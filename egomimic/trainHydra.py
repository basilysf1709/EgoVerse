...
def train(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    # every resolver before instantiation so each ZarrDataset loads keep_indices
    # from the cache (by episode hash) instead of recomputing the pause mask
    # in-process. Covers train, valid, and train_viz datasets.
    _pause_cache = OmegaConf.select(cfg, "data.pause_precompute_cache", default=None)
    if _pause_cache:
        for _block_name in ("train_datasets", "valid_datasets", "train_viz_datasets"):
            _block = OmegaConf.select(cfg.data, _block_name) or {}
            for _ds_name, _ds_cfg in _block.items():
                if (
                    _ds_cfg is not None
                    and OmegaConf.select(_ds_cfg, "resolver") is not None
                ):
